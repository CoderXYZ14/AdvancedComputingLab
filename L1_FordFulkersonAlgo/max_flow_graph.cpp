...
private:
    ...
    bool breadthFirstSearch(int source, int sink, std::vector<int> &parent)
    {
...
public:
    MaxFlowSolver(int v) : vertices(v), graph(v, std::vector<int>(v, 0)),
                           residualGraph(v, std::vector<int>(v, 0)) {}
...
static void display_steps(GtkWidget *widget, gpointer data)
{
    ...
    if (solver)
    {
        const std::vector<std::string> &steps = solver->getSteps();
...
static void run_max_flow(GtkWidget *widget, gpointer data)
{
    ...
    int source = std::stoi(sourceText);
    int sink = std::stoi(sinkText);
    ...
    // Parse and add edges
    std::istringstream edgeStream(edgeText);
    int u, v, capacity;
